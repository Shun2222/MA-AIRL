...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        # set initial states
        for i, agent in enumerate(world.agents):
            agent.state.p_pos =  np.array(self.env_info.discrete_to_continue(starts_pos[i]), dtype='float64')
            #agent.state.p_pos = np.random.uniform(-1, +1, world.dim_p)
            agent.state.p_vel = np.zeros(world.dim_p)
            agent.state.c = np.zeros(world.dim_c)
        for i, landmark in enumerate(world.goals):
            landmark.state.p_pos = np.array(self.env_info.discrete_to_continue(goals_pos[i]), dtype='float64')

...
def render(env, image, all, path, discrete, grid_size):
    ...
    for i in range(num_trajs):
        ...
        g = 0
        aact0 = [1]*14 + [4]*14 + [0 for _ in range(50)]
        aact1 = [2]*14 + [4]*14 + [0 for _ in range(50)]
        aact = [aact0, aact1]
        while not done:
            #0 stay 1 right 2 left 3 up 4 down

            
            actions_list = [onehot(int(action[k][0]), n_actions[k]) for k in range(n_agents)]
            # actions_list = [onehot(np.random.randint(n_actions[k]), n_actions[k]) for k in range(n_agents)]
            action = [[aact0[g]], [aact1[g]]]
            g+=1
            print(action)
